...
for j in range(1,18):
    ...
    for fileName in filenames_torque:
        ...
        horizontal_stack['knee_r_power'+col_add] = 0.0174533*horizontal_stack['knee_angle_r_moment'+col_add]*(horizontal_stack['knee_angle_r'+col_add] - horizontal_stack['knee_angle_r_prev'+col_add])/(horizontal_stack['time']-horizontal_stack['time_prev'])
        horizontal_stack['ankle_r_power'+col_add] = 0.0174533*horizontal_stack['ankle_angle_r_moment'+col_add]*(horizontal_stack['ankle_angle_r'+col_add] - horizontal_stack['ankle_angle_r_prev'+col_add])/(horizontal_stack['time']-horizontal_stack['time_prev'])
        ...
        horizontal_stack['knee_l_power'+col_add] = 0.0174533*horizontal_stack['knee_angle_l_moment'+col_add]*(horizontal_stack['knee_angle_l'+col_add] - horizontal_stack['knee_angle_l_prev'+col_add])/(horizontal_stack['time']-horizontal_stack['time_prev'])
        horizontal_stack['ankle_l_power'+col_add] = 0.0174533*horizontal_stack['ankle_angle_l_moment'+col_add]*(horizontal_stack['ankle_angle_l'+col_add] - horizontal_stack['ankle_angle_l_prev'+col_add])/(horizontal_stack['time']-horizontal_stack['time_prev'])
        
        cols = ['time_prev_prev','time_prev','time','hip_r_power'+col_add,'knee_r_power'+col_add,'ankle_r_power'+col_add,'hip_l_power'+col_add,'knee_l_power'+col_add,'ankle_l_power'+col_add]
        horizontal_stack = horizontal_stack.drop(horizontal_stack.columns.difference(cols), axis=1)
        hip_pos = (horizontal_stack['hip_r_power'+col_add] + abs(horizontal_stack['hip_r_power'+col_add]))/2 + (horizontal_stack['hip_l_power'+col_add] + abs(horizontal_stack['hip_l_power'+col_add]))*0.005/2
        hip_neg = (-horizontal_stack['hip_r_power'+col_add] + abs(horizontal_stack['hip_r_power'+col_add]))/2 + (-horizontal_stack['hip_l_power'+col_add] + abs(horizontal_stack['hip_l_power'+col_add]))*0.005/2
        knee_pos = (horizontal_stack['knee_r_power'+col_add] + abs(horizontal_stack['knee_r_power'+col_add]))/2 + (horizontal_stack['knee_l_power'+col_add] + abs(horizontal_stack['knee_l_power'+col_add]))*0.005/2
        knee_neg = (-horizontal_stack['knee_r_power'+col_add] + abs(horizontal_stack['knee_r_power'+col_add]))/2 + (-horizontal_stack['knee_l_power'+col_add] + abs(horizontal_stack['knee_l_power'+col_add]))*0.005/2
        ankle_pos = (horizontal_stack['ankle_r_power'+col_add] + abs(horizontal_stack['ankle_r_power'+col_add]))/2 + (horizontal_stack['ankle_l_power'+col_add] + abs(horizontal_stack['ankle_l_power'+col_add]))*0.005/2
        ankle_neg = (-horizontal_stack['ankle_r_power'+col_add] + abs(horizontal_stack['ankle_r_power'+col_add]))/2 + (-horizontal_stack['ankle_l_power'+col_add] + abs(horizontal_stack['ankle_l_power'+col_add]))*0.005/2
        pow_tot = (pow_tot  + ((abs(horizontal_stack['hip_r_power'+col_add]).sum() + abs(horizontal_stack['hip_l_power'+col_add]).sum() +
                   abs(horizontal_stack['knee_r_power'+col_add]).sum() + abs(horizontal_stack['knee_l_power'+col_add]).sum() +
                   abs(horizontal_stack['ankle_r_power'+col_add]).sum() + abs(horizontal_stack['ankle_l_power'+col_add]).sum()
                    )*0.005)/horizontal_stack['time'][horizontal_stack.shape[0]-1])
        Total_positive = Total_positive + (((abs(horizontal_stack['hip_r_power'+col_add])+horizontal_stack['hip_r_power'+col_add]).sum() + (abs(horizontal_stack['hip_l_power'+col_add])+horizontal_stack['hip_l_power'+col_add]).sum() +
                   (abs(horizontal_stack['knee_r_power'+col_add])+horizontal_stack['knee_r_power'+col_add]).sum() + (abs(horizontal_stack['knee_l_power'+col_add])+horizontal_stack['knee_l_power'+col_add]).sum() +
                   (abs(horizontal_stack['ankle_r_power'+col_add])+horizontal_stack['ankle_r_power'+col_add]).sum() + (abs(horizontal_stack['ankle_l_power'+col_add])+horizontal_stack['ankle_l_power'+col_add]).sum()
                   )*0.0025)/horizontal_stack['time'][horizontal_stack.shape[0]-1]
        Total_negative = Total_negative + (((abs(horizontal_stack['hip_r_power'+col_add])-horizontal_stack['hip_r_power'+col_add]).sum() + (abs(horizontal_stack['hip_l_power'+col_add])-horizontal_stack['hip_l_power'+col_add]).sum() +
                   (abs(horizontal_stack['knee_r_power'+col_add])-horizontal_stack['knee_r_power'+col_add]).sum() + (abs(horizontal_stack['knee_l_power'+col_add])-horizontal_stack['knee_l_power'+col_add]).sum() +
                   (abs(horizontal_stack['ankle_r_power'+col_add])-horizontal_stack['ankle_r_power'+col_add]).sum() + (abs(horizontal_stack['ankle_l_power'+col_add])-horizontal_stack['ankle_l_power'+col_add]).sum()
                   )*0.0025)/horizontal_stack['time'][horizontal_stack.shape[0]-1]
        # print(id_num)
...
pow_tot = pow_tot/count_runs
Total_positive = Total_positive/count_runs
Total_negative = Total_negative/count_runs
print("Total Power on Average: ", pow_tot)
print("Total Positive Power On Average: ", Total_positive)
print("Total Negative Power On Average: ", Total_negative)
print("Total Count: ", count_runs)
horizontal_stack_overall.to_csv("Power_data_overall.csv", sep=',')